...
void TSolderingIronController::DisplayLoop()
{
  ...
  if( NotificationValue & EventPrint )
  {
    ...
    UartPrintf( "%6u %d %u.%03uA %2u.%uV %2u.%uV %u %d\n", Ambiant, Heat, Ain1 / 1000, Ain1 % 1000, Vin1 / 10, Vin1 % 10, Vin0 / 10, Vin0 % 10, Power, HandleSwitch );
  }
...
void TSolderingIronController::ApplicationTickHook()
{
  DeadtimeEncA >>= 1;
  DeadtimeEncB >>= 1;
  DeadtimeEncEnter >>= 1;
...
{
  SolderingIronController.ApplicationTickHook();
}
